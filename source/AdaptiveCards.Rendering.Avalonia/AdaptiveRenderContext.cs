...
namespace AdaptiveCards.Rendering.Avalonia
{
    ...
    public class AdaptiveRenderContext
    {
            ...
            foreach (AdaptiveTargetElement targetElement in targetElements)
            {
                ...
                if (RenderedElementsWithId.TryGetValue(targetElement.ElementId, out element))
                {
                    if (element != null && element is Control elementControl)
                    {
                        ...
                        TagContent tagContent = GetTagContent(elementControl);
        ...
        private Control GetRenderedElement(Control element)
        {
            ...
            {
                Control uiElement = containerPanel.Children[0];

                if (uiElement is Control frameworkElement)
                {
        ...
        {
            Control card = ActionShowCards[uiAction];
