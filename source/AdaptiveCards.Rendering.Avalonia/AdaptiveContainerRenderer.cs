...
namespace AdaptiveCards.Rendering.Avalonia
{
    public static class AdaptiveContainerRenderer
    {
                ...
                if (rendereableElement != null)
                {
                    ...
                    if (uiElement != null)
                    {
                        ...
                        if (cardElement.Height != AdaptiveHeight.Stretch)
                        {
                            ...
                            Control enclosingElement = EncloseElementInPanel(context, rendereableElement, uiElement);
        ...
        public static Control EncloseElementInPanel(AdaptiveRenderContext context, AdaptiveTypedElement element, Control renderedElement)
        {
            Control enclosingElement = renderedElement;
            ...
            if (element is AdaptiveInput)
            {
                ...
                Control elementForAccessibility = renderedElement;
                ...
                if (inputElement is AdaptiveChoiceSetInput)
                {
                    if (renderedElement is Grid)
                    {
                        ...
                        Control choiceSet = inputContainer.Children[0];

                        if (choiceSet is ComboBox)
                        {
                            elementForAccessibility = choiceSet as Control;
                        }
                        else if (choiceSet is Panel)
                        {
                            // If it's a choice set, then use the first element as element
                            elementForAccessibility = ((choiceSet as Panel).Children[0] as Control) ?? renderedElement;
                        }
                    }
                }
        ...
        public static Control EncloseInputForVisualCue(AdaptiveRenderContext context, AdaptiveInput element, Control renderedElement)
        {
            Control enclosingElement = renderedElement;
